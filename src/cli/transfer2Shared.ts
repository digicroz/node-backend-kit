...
export const transfer2Shared = async () => {
  ...
  try {
    ...
    for (const project of b2fPortalProjects) {
      const projectSpinner = startProgress(
      ...
      try {
        ...
        if (project.sharedBackendPath) {
          ...
          const sharedBackendChanges = sharedBackendGitStatus.files.some(
            (file: { path: string }) =>
          ...
          if (!sharedBackendChanges) {
            ...
            projectProgress.incrementCompleted(
              `Completed: ${project.projectName} - No changes`
            )
            continue
          }

          const repositoryCommitJsonPath = join(
            sharedBackendPath,
            "commit.json"
          )
          ...
          if (!fs.existsSync(repositoryCommitJsonPath)) {
            failProgress(
              projectSpinner,
              "repositoryCommitJsonPath does not exist"
            )
            console.log({ sharedBackendPath })
            projectProgress.incrementFailed(`Failed: ${project.projectName}`)
            return
          ...
          successProgress(
            projectSpinner,
            `Changes committed and pushed successfully for project: ${project.projectName}`
          )
          projectProgress.incrementCompleted(
        ...
        failProgress(
          projectSpinner,
          `Error processing project: ${project.projectName}`
        )
        projectProgress.incrementFailed(`Failed: ${project.projectName}`)
