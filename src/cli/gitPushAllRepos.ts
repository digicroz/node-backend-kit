...
async function commitB2fPortalChanges(repoPath: string) {
  ...
  try {
    ...
    const b2fPortalChanges = status.files.some((file: { path: string }) =>
      file.path.startsWith("b2fPortal/")
    )

    if (!b2fPortalChanges) {
      console.log(
        `No changes detected in the b2fPortal directory. Skipping commit.`
      )
      return
    }
  ...
  for (const project of b2fPortalProjects) {
    for (const section of project.sections) {
      if (isDeveloperAdarsh) {
        ...
        console.log(
          `----------------${section.repository.name} finished----------------\n`
        )
      }
    }
  }
