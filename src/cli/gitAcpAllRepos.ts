...
const runAcpCommands = async (repoPath: string, message: string) => {
  try {
    ...
    const spinner = startProgress(
      `Running ACP in ${repositoryPackageJson.name}`
    )
    ...
    updateProgress(
      spinner,
      `${repositoryPackageJson.name}: Checking status ...`
    )
    ...
    if (status.files.length > 0) {
      ...
      updateProgress(
        spinner,
        `${repositoryPackageJson.name}: Committing changes ...`
      )
    ...
    if (status.files.length > 0) {
      updateProgress(
        spinner,
        `${repositoryPackageJson.name}: Adding files after pull ...`
      )
      await git.add(".")
      updateProgress(
        spinner,
        `${repositoryPackageJson.name}: Committing changes after pull ...`
      )
      await git.commit(message)
    }
    ...
    successProgress(
      spinner,
      `Running ACP in ${repositoryPackageJson.name} Finished`
    )
  } catch (error: any) {
...
export const gitAcpAllRepos = async () => {
  ...
  try {
    ...
    for (const repoPath of allPaths) {
      try {
        await runAcpCommands(
          repoPath,
          `${new Date().toLocaleString()} : ${commitMessage}`
        )
