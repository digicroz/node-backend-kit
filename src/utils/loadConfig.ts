...
${yellow}Please create a pbk.config.json file at the root of your project with the following structure:${reset}

${configExample}

${yellow}To specify a different config location, provide the path when calling pbkInit():${reset}
${cyan}pbkInit({ configPath: './path/to/your/config.json' })${reset}
 ...
 */
export function loadConfig(configPath: string = path.join(process.cwd(), 'pbk.config.json')): TPbkConfig {
    try {
 ...
 */
function validateConfig(config: any): asserts config is TPbkConfig {
 ...
 */
function validateProject(project: any, index: number): asserts project is TPbkProject {
  // Required fields
 ...
 */
function validateSection(section: any, projectName: string, index: number): asserts section is TPbkProjectSection {
  // Required fields
  ...
  if (section.needNextJsPatch !== undefined && typeof section.needNextJsPatch !== 'boolean') {
